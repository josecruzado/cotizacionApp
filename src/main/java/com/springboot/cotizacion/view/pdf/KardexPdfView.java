package com.springboot.cotizacion.view.pdf;
...
public class KardexPdfView extends AbstractPdfView {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		Cliente cliente = (Cliente) model.get("kardex");
		...
		cell = new PdfPCell(new Phrase(messageSource.getMessage("text.kardex.ver.datos.cliente", null, locale)));
		cell.setBackgroundColor(new Color(184, 218, 255));
		cell.setPadding(8f);
		tabla.addCell(cell);
		
		
		tabla.addCell(cliente.getId() + " , Cantidad máxima:" + cliente.getId()+", Cantidad minima: "+ cliente.getNumero());
		...
		cell = new PdfPCell(new Phrase(messageSource.getMessage("text.kardex.ver.datos.factura", null, locale)));
		...
		*/
		...
		/*
		for(DetalleKardex item: kardex.getDetalles()) {
			tabla3.addCell(item.getConcepto());
			tabla3.addCell(item.getNroDoc());

			cell = new PdfPCell(new Phrase(item.getCantidad()));
